...
def application():
    ...
    zero = cv2.resize(
        cv2.imread(os.path.join(os.getcwd(), "uploads", "1.jpg")), (1920, 1080)
    )
    one = cv2.resize(
        cv2.imread(os.path.join(os.getcwd(), "uploads", "2.jpg")), (1920, 1080)
    )
    two = cv2.resize(
        cv2.imread(os.path.join(os.getcwd(), "uploads", "3.jpg")), (1920, 1080)
    )
    three = cv2.resize(
        cv2.imread(os.path.join(os.getcwd(), "uploads", "4.jpg")), (1920, 1080)
    )
    four = cv2.resize(
        cv2.imread(os.path.join(os.getcwd(), "uploads", "5.jpg")), (1920, 1080)
    )
    five = cv2.resize(
        cv2.imread(os.path.join(os.getcwd(), "uploads", "6.jpg")), (1920, 1080)
    )
    six = cv2.resize(
        cv2.imread(os.path.join(os.getcwd(), "uploads", "7.jpg")), (1920, 1080)
    )

    # uploading background image

    background = cv2.resize(
        cv2.imread("static/images/background.png"),
        (1920, 1080),
    )
        ...
        if hand:
            ...
            if lmlist:
                ...
                if fingerup == [0, 0, 1, 1, 1]:
                    ...
                    cv2.destroyWindow("frame1")
                    buf = cv2.rotate(
                        images[index], cv2.ROTATE_90_COUNTERCLOCKWISE
                    )
                    print(fing)
                    time.sleep(1)
